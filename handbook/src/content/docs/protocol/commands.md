...
| `0x02/0x18` | **resolution + fps** | 5 B `[res][fps_idx] 00 00 00`. Aspect is the **res byte**, not a second SET (`0A` 1080p 16:9 / `10` 4K 16:9; media catalog also `2D` 2.7K 16:9, `0C`/`5F`/`67` 4:3, `69`/`6A`/`6B` 1:1, `42`/`43`/`6C` 9:16). fps `01`=24 `02`=25 `03`=30 `04`=48 `05`=50 `06`=60; SlowMo `07`=120 `08`=240 `0A`=100 (200 unlabeled). No GET — `cam_video_param_v2` `@0–1`. Legal pairs for the current shooting mode arrive as `camcap_video_format` (`01` + inner u16-LE + count + count×`[res][fps][00]`). Pocket 4 Pro Video (`mimo-live-start-20260828`) is 4K then 1080p, 24–60 only. SET SlowMo indices only after that mode’s camcap take. |
