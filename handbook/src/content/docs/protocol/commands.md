...
| `0x02/0x42` | **color mode** | Wheel follows the body. Pocket 4 Pro `3F` Normal / `3C` HDR / `17` D-Log / `41` D-Log2. Pocket 4 `3F` / `3C` / `17` D-Log (no D-Log2). Pocket 3 `3F` / `3C` HDR (HLG) / `00` D-Log M — `17` is not D-Log M. Nano `camcap_color_mode` `00 3F 3D` = D-Log M / Normal 8-bit / Normal 10-bit. No GET — `cam_image_effect` `@2` |
| `0x02/0x18` | **resolution + fps** | 5 B `[res][fps_idx] 00 00 00`; `0A` 1080p / `10` 4K; fps `01`=24 `02`=25 `03`=30 `04`=48 `05`=50 `06`=60; no GET — `cam_video_param_v2` `@0–1`. Legal pairs for the current shooting mode arrive as `camcap_video_format` (`01` + inner u16-LE + count + count×`[res][fps][00]`). Pocket 4 Pro Video (`mimo-live-start-20260828`) is 4K then 1080p, 24–60 only. SlowMo 100/120/240 is a different mode; SET those indices only after a labeled SlowMo take. |
| `0x02/0xb8` | **zoom SET** | slider `0A 4E` + lens `@14` (217 = 1×, 651 = 3×, 2604 = 12× on Pocket 4 Pro). Chip cycle follows the body: 4 Pro 1×/3×/6×/12×; Pocket 4 / 3 1×/2×/4× (Pocket 3 4K Video max 2×, DJI spec); Nano 1×. SlowMo / TimeLapse / SuperNight: digital zoom off (Pro keeps 1×/3× optical). Mimo 1×→12× pinch is this form only, ~20 Hz. Older takes also have slew `03 00` + 100/300. ACK `00`. No GET. **D-Log2 rejects every zoom SET** — hop `0x02/0x42` to D-Log (`17`) first and wait for `cam_image_effect` `@2` before `0xB8` |
...
| `0x02/0x2C` | **white balance** | 5 B `[mode][K/100 u16-LE][tint i16-LE]`. Auto `00` keeps tint (kelvin 0; Mimo `00 00 00 14 00` at +20). Custom `06`. ACK `00` flags `0xc0` ~10 ms. One in flight (100 ms coalesce) — do not flood. No GET — `cam_image_effect` `@4` mode, `@5–8` Custom K/tint. Auto `@5–6` is live-measured; do not SET it. Named presets are app Kelvin shortcuts, not extra mode bytes. |
...
| `0x04/0x05` | **gimbal attitude** | unsolicited ~10 Hz, 50 B. i16-LE @4 in 0.1° yaw; i16-LE @6 in 0.1° pitch (consecutive field after proven yaw). Absolute yaw > 90° is the selfie-facing pose. Joystick pan and `FE 09` both move it. Pitch `@6` is the field that moves when commanding tilt — confirm on a physical Pocket if a take disagrees |
