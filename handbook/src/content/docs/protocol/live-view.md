...
In the available single-client captures, that media is **unicast** to one client 5-tuple: camera `192.168.2.1:9004` → the phone's camera DHCP IPv4 and an ephemeral local port. No camera multicast or broadcast HEVC/AVC was observed. Simply joining the SoftAP does not duplicate that flow; a second client's handshake behavior remains untested. The app keeps one camera client. Operator Setup → Sharing stays parked. A watcher feed would be a phone-side relay on another interface, not camera multicast.
