...
class NativeProgramZoomTest {
    ...
    @Test fun timedLegKeepsOneSpeedWithoutPositionStepsOrStopPulses() {
        val dt = 0.0005
        var distance = 0.0
        ...
        repeat(5_000) {
            ...
            } else assertTrue(commands.isEmpty() || commands.last() == NativeProgramZoomCommand.Stop,
                "The waiting interval precedes one uninterrupted zoom")
            if (commands.lastOrNull() != command) commands += command
        }
        assertEquals(listOf<NativeProgramZoomCommand>(NativeProgramZoomCommand.Stop, NativeProgramZoomCommand.Rate(73, true)), commands)
    ...
    @Test fun crossingAnEpsilonBeforeBPreservesTheNextLegStartDeadline() {
        val a = GimbalWaypoint(0.0, 0.0, 3.0, 175.0)
        val path = GimbalZoomPath(GimbalProgram(a, a.copy(zoom = 6.0), a, 2.5, 2.5))
        val demand = path.nativeDemand(2.5 - 5e-10)
        assertEquals(path.nativeDemand(2.5), demand)
        assertEquals(NativeProgramZoomCommand.Stop, demand.command)
        assertEquals(2.5 - ln(2.0) / (2 * NativeProgramZoom.SLOWEST_LOG_RATE), demand.nextChange, 1e-9)
        assertNull(demand.failureReason)
        assertTrue(demand.nextChange > 0)
    }

    @Test fun eachLegUsesOneRateSpanWithTheExactDistanceAndDeadline() {
