...
subject loss. The revision below addresses reproduced controller resets and
single-frame loss, and uses the existing face detector continuously for people.
...
and a critically damped speed spring with acceleration and jerk bounds. Rate
feedback adds braking to damp the complete delayed camera/image loop; a critically
damped speed filter alone does not make that loop critically damped. The
...
Reaching that lead limit preserves easing state; resetting it caused repeated
acceleration/braking cycles with delayed attitude. Mechanical reach limits still
reset the affected axis. A detection hold sends the relative native stop, never
...
does a synthetic camera loop with 160 ms video delay, 80 ms quantized attitude
delay and 100 ms motor response across all presets. These are simulation
parameters, not measured Pocket latency. Tests also cover bounded recovery,
