...
| Phone-side cinematic tracking prototype | `CinematicTrackingController` (noise filter, subject-velocity matching, dead band, lerp, damped trajectory and delayed-progress watchdog), `CinematicFaceLock` (short spatial association) | iOS `CinematicTrackingPrototype` owns object Vision, frame admission and session generations; faces share `LiveFaceDetector` with fresh raw measurements. `CameraSession` grants exclusive native targets after ActiveTrack clear; short loss sends a token-fenced stop retaining ownership. `LiveCinematicTracking` presents Gimbal → Track. Android is deferred under the parity exception. [Contract](cinematic-tracking.md). |
