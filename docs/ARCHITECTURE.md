...
| AirPods look-at gimbal | `HeadTrack` (SET-relative nose look = pan/tilt; swing-only error onto a predicted pose; live `0x04/0x05` is a snap when attitude jumps ahead) | iOS `HeadphoneMotionBridge`. Roll readout only. Android: no IMU — PARITY exception. |
