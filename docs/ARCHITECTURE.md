...
| Phone-side cinematic tracking prototype | `CinematicTrackingController` (noise filter, dead band, lerp, bounded motion) | iOS `CinematicTrackingPrototype` owns Vision, frame admission and session generations; `CameraSession` grants exclusive native targets after ActiveTrack clear. `LiveCinematicTracking` presents Gimbal → Track. Android is deferred under the parity exception. [Contract](cinematic-tracking.md). |
