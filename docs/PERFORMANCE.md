...
| Face AF | Latest-wins, one in flight: 25 Hz while a face is present, 10 Hz after about one second (25 runs) with none; the first face restores 25 Hz. iOS runs Vision on the live VT buffer. Android asks the detector before any readback (`wantsFrame`), reads back only when it will be used, and hands ML Kit 320×180 NV21 converted in native code (ML Kit's own Bitmap path converted in Java at about 40% of Face AF CPU; 320 found the same faces as 640 down to 8% of frame width in 30 to 54% less time). | `LiveFaceDetector.pace` (both shells) |
