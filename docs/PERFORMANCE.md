...
| Cinematic Tracking prototype (iOS) | Off by default. Faces share the full-source AF detector, ≤25 Hz, one job including delivery plus one replaceable newest frame; admission never queues each frame behind Vision. Objects use one Vision job, drop occupied frames, 640 px longest side, ≤15 Hz. Either tracking path reduces to 5 Hz at serious thermal state; critical stops tracking. Original frame timestamps expire for motion at 250 ms; one-second bounded selection recovery; control 25 Hz with 120 ms gap stop. Bounded pose/target histories (≤64 each) and bearing regression (≤16 samples) run on the existing control loop. Native target/ACK cadence unchanged; a hold sends one relative stop. Acquired box/readouts ≤5 Hz. Revised physical live-rate/thermal qualification pending. | `CinematicTrackingPrototype`, `LiveFaceDetector`, [contract](cinematic-tracking.md) |
