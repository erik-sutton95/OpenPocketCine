...
| Cinematic Tracking prototype (iOS) | Off by default; one Vision job including result delivery; drop frames while occupied; 640 px longest side; ≤15 Hz (5 Hz serious thermal, stop at critical). Original frame timestamps expire at 250 ms; control 25 Hz with 120 ms gap stop; native target mailbox and ACK cadence unchanged. Acquired box/readouts ≤5 Hz. Physical live-rate and thermal qualification pending. | `CinematicTrackingPrototype`, [contract](cinematic-tracking.md) |
