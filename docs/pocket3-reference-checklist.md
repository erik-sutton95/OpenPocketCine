...
| Zoom and Med-Tele | Held zoom reaches 2.0× at 4K, 3.0× at 2.7K and 4.0× at 1080P. Med-Tele visibly changes framing and resets its displayed relative zoom; ISO MAX ceiling 1600. Candidate request mapping recorded. Med-Tele *state* is read back from `cam_status` `@5`, `cam_lens_state` `@10`/`@12` and `cam_fov` `@12`, and drives a 2×/3×/4× range (physical, 2026-09-20). | Calibrated focal length/optical behavior, all mode constraints and reconnect persistence. Entering or leaving Med-Tele from the app (the `0x02/0xFF` mapping stays a candidate, unexercised). The 2.7K clamp discrepancy is closed — per-FORMAT ceilings ship. |
