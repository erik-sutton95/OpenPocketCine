...
| Diagnostics | Operator Setup → System **and** Connection setup (first pair) → **Share Diagnostics** (redacted report). Journal in app documents. Typed feed-incident spool is local on both shells (Share extras, bounded retention). | iOS copies a compact paste on screenshot for TestFlight feedback (Apple cannot attach files to that form). Android has no TestFlight screenshot hook — Share only. MetricKit is iOS. Automatic reliability reports (consent + https DSN) are **iOS-only** and **not configured** in this tree — no DSN, no proven upload. Android has the local spool only. | Local spool: unit tests. **Physical qualification pending both.** No Android device attached; iOS baseline attempted but blocked by automation timeout / camera Wi-Fi join error 8; zero live exposure. |
| Decoder-output recovery | Fresh complete AUs + silent native output: one decoder rebuild and one enable; retain last image; 16 s picture deadline then datalink rejoin. Fresh native output does not PLI. Blocked enable is not a spent rung. Settings cover does not drop `0x02` ingest. | iOS VideoToolbox vs Android MediaCodec. Packet-without-complete-AU stall uses the existing enable ×2 / endpoint ladder (portable tests). Renderer-only local repair is **not implemented**. Seeded physical stress harness is **iOS Debug XCTest only**. | **Not qualified.** Portable `FeedWatchdog` / incident tests only. No new physical iOS camera take. No attached Android device. |
