...
| Head tracking | iOS: Controls **Head Tracking (Experimental)** (off by default). AirPods with motion. Live **Calibrate Head Lock** (centered above the bottom bars) is shared forward — that head pose and that gimbal pose are zero. Look is the SET-relative nose (CMAttitude +X right, +Y forward, +Z up), not Euler yaw. Gimbal is reconstructed the same way; error is swing (nose-to-nose), not a 3DOF quaternion SET (Pocket has none; roll is not on `0x04/0x01`). Stick closes a predicted pose (throw × 120°/s, error/10° up to full Mimo ±550) onto a committed look; live `0x04/0x05` snaps the predictor only when it jumps ahead of the throw. Retarget only when the head moves ≥2.5° — turn the other way before the gimbal arrives. Do not reverse-hunt on delayed attitude. Gimbal Fast + tilt unlocked at calibrate. A 20° head turn is a 20° gimbal turn. Roll is shown, not driven. STOP clears SET. Chip/stick/gamepad win while thrown. Toast if IMU is missing or the head is moving at calibrate. Live debug: yaw ring (12 o'clock is SET) and a vertical pitch ring (arrow-right is 0); white arrow is the head, sky arrow is live gimbal pan/tilt. | Android has no AirPods IMU — no Controls row | **physical** iOS |
