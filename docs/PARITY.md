...
| Operator Setup | Seven tabs (Link, Sharing, View Assist, Controls, Display, Storage, System); DJI Black; Sora + IBM Plex; NOTICE legal | Frame.io row is “Not configured” until iOS keys exist. Sharing browse / advertise / join / control is **iOS-only** (Bonjour `_opc-mon._tcp`, AWDL while the host is on SoftAP). Android Sharing stays Coming soon until a second interface exists. | **physical** iOS for Sharing; both for the other six tabs |
