...
Both shells offer Report a problem (private email), Automatic error reports,
offline Reporting Privacy, and collapsed Diagnostic options. Public GitHub report
and feature-request actions are removed from this section. iOS mail fallback uses
a compact message when the native attachment composer is unavailable; Android
uses an email-only chooser with recipient and attachments. Physical iPhone navigation passed for the compact section, expanded options and
offline privacy page. Email handoff remains pending; no Android device was attached
for physical navigation or email verification.
