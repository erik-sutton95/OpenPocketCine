...
| --- | --- | --- | --- |
| Shooting-mode capture | Photo (`05` / `17`) uses the shutter with no recording confirmation; Low-Light / SuperNight (`28`) is video. Mode, recording, connection or lock changes dismiss pending confirmation; a mode change invalidates stale format options. Photo does not offer video FPS or shutter angle. Reported format pairs take priority; Pocket 3 has documented Video, Slow Motion and Low-Light fallbacks. Other mode/body combinations require camera capabilities. | Native picker and confirmation rendering. Pocket 4 Pro Mimo comparison is being prepared; Pocket 3 is no longer available and regular Pocket 4 is untested in this pass. | **Physical qualification pending both shells.** iPhone XCTest startup blocked by “Timed out while enabling automation mode”; no new Pocket 4 Pro survey evidence collected yet. |
