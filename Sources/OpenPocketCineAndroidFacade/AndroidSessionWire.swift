...
public enum AndroidSessionWire {
    ...
    public static func status(fromJSON json: String) -> CameraStatus {
        ...
        let lensMin = int("zoomLensMin", default: -1)
        if (0...65_535).contains(lensMin) { status.zoomLensMin = UInt16(lensMin) }
        let lensMax = int("zoomLensMax", default: -1)
        if (0...65_535).contains(lensMax) { status.zoomLensMax = UInt16(lensMax) }
