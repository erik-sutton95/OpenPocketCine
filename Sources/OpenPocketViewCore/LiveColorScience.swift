...
public enum LiveColorScience {
    ...
    public static func falseColorBands(
        ...
        switch scale {
        case .stops: stopBands(transfer: transfer, clipEncoded: clipEncoded)
        case .ire: ireBands
...
extension LiveColorScience {
    ...
    fileprivate static func stopBands(transfer: MonitorTransfer, clipEncoded: Double? = nil)
        -> [LiveFalseColorBand]
    {
        let clipLinear = linearize(
            clipEncoded ?? ScopeExposureCeiling.clipEncoded(transfer: transfer), transfer: transfer)
