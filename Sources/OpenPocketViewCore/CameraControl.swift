...
public enum CamFov {
    ...
    /// clamps a wider ask back to the floor, so the tap would go nowhere. Getting
    /// back to 1x is the MT button's job — taking the lens off — not a stop. Nil
    /// when Med-Tele is off or the body has not reported its limits yet, and the
    /// caller keeps the per-FORMAT table.
    ...
    /// before telling the operator it did not happen.
    ///
    ...
    /// would look like a slow one.
    public static let medTeleSwapTimeout: TimeInterval = 2

    /// Whether the MT button may take the Pocket 3's Med-Tele lens on or off
    /// right now.
    ///
    /// These are the states where the swap was measured not to work, each a
    /// *silent* refusal — the body neither moves nor NACKs — so the button is
    /// dimmed there instead of spending a tap on nothing:
    ...
    /// clears tracking itself rather than dimming a button the body would honour.
    public static func medTeleToggleable(
        colorMode: ColorMode?, isRecording: Bool, shootingMode: Int
    ) -> Bool {
        colorMode == .normal && !isRecording
