    ...
    struct LucideSVGDocument {
        ...
        private init(viewBox: CGRect, strokeWidth: CGFloat, elements: [Element]) {
            ...
            outline = Self.makePath(elements)
        }

        func draw(in context: inout GraphicsContext, size: CGSize, filled: Bool) {
            let path = combinedPath()
            guard !path.isEmpty, viewBox.width > 0, viewBox.height > 0, size.width > 0,
                size.height > 0
            else { return }
        ...
        /// Compile SVG commands once with the resource, not on every HUD redraw.
        private static func makePath(_ elements: [Element]) -> Path {
            var path = Path()
        ...
        private static func resourceURL(named name: String) -> URL? {
            Bundle.module.url(forResource: name, withExtension: "svg", subdirectory: "Icons/lucide")
        }
    ...
    enum LucideSVGCache {
        ...
        static func document(named name: String) -> LucideSVGDocument? {
            storage.document(named: name)
        }

        /// The lock protects the complete lookup/load/insert transaction. Parsed
        /// documents are immutable values; no rendering work runs under this lock.
        private final class Storage: @unchecked Sendable {
            private let lock = NSLock()
            private var documents: [String: LucideSVGDocument] = [:]

            func document(named name: String) -> LucideSVGDocument? {
                lock.lock()
                defer { lock.unlock() }
