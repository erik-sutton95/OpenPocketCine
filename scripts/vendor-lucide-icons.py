...
AUTO_MIRROR = {"chevron-left", "chevron-right"}
SVG_URL = (
...
def vector_xml(icon: str, paths: list[str]) -> str:
    mirror = (
...
def vendor(from_dir: Path | None) -> None:
    ...
    for name in ICON_NAMES:
        if from_dir is not None:
            svg_bytes = (from_dir / f"{name}.svg").read_bytes()
        else:
            svg_bytes = fetch(SVG_URL.format(name=name))
        ...
        xml = vector_xml(name, element_paths(root))
