...
struct FalseColorReference: View {
    ...
    static func stopAxisMarkers(transfer: MonitorTransfer) -> [AxisMarker] {
        axisMarkers(
            [
                ...
                ("Skin", 1),
                ("+2", 2),
                ("Max", PocketFalseColorMap.maximumSceneStop(transfer: transfer)),
            ],
            domain: stopReferenceDomain(scale: .stops, transfer: transfer))
    }

    static func elZoneAxisMarkers() -> [AxisMarker] {
        axisMarkers(
            [("−6", -6), ("−3", -3), ("18%", 0), ("+3", 3), ("+6", 6)],
            domain: elZoneReferenceDomain)
    }

    private static let elZoneReferenceDomain: ClosedRange<Double> = -6.5...6.5

    private static func axisMarkers(
        _ markers: [(String, Double)], domain: ClosedRange<Double>
    ) -> [AxisMarker] {
        markers.enumerated().map { index, marker in
            AxisMarker(
    ...
    private static func stopReferenceDomain(
        ...
        if scale == .elZone { return elZoneReferenceDomain }
